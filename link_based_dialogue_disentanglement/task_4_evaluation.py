...
def link_score(gedges, aedges):
    # recall
    deno_gold = 0
    deno_auto = 0
    num = 0
    for k, gold in gedges.items():
        golds = set()
        autos = set()
        for child, parents in gold.items():
            for parent in parents:
                golds.add((parent, child))
        for child, parents in aedges[k].items():
            for parent in parents:
                autos.add((parent, child))
        num += len(golds.intersection(autos))
        deno_gold += len(golds)
        deno_auto += len(autos)
    ...
    p = num/deno_auto*100
    f = 2*p*r/(p+r)
    print("{:5.2f}   Matched links precision".format(p))
    print("{:5.2f}   Matched links recall".format(r))
    print("{:5.2f}   Matched links f-score".format(f))
    return f
...
if __name__ == '__main__':
    ...
    if 'link' in args.metric or 'all' in args.metric:
        link_score(gedges, aedges)
